...
Signal was endorsed by the cryptographic community in several occasions and is by many considered as the most secure messenger. It's the only messenger completely open-sourced in this collection.
...
Attacker then created a fake HTTP OK response to let the messenger think the request was successful and entered retrieved verification code from the intercepted request. The attacker successfully hijacked the victim's WhatsApp identity and could both send and recieve all his messages.
...
Those are fully readable for telecom operators which may and usually do cooparate with the corresponding goverment. Furthermore SMS can be intercepted using inexpensive IMSI catchers. If an attacker is able to steal the authenticating SMS he is able to login on his behalf. Worse, Telegram stores the whole user's message history, hence it allows the attacker to read old messages as well.
...
Telegram authors decided to craft a brand new encryption scheme supposedly to achieve better delivery times and stability, MTProto. For simplicity we will focuse on Telegram's secret chats messaging. The analysis is mainly based on \cite{telegram-aarhus} and on the official Telegram documentation. 
...
The exchange performs traditional Diffie-Hellman key exchange (DHKE). Using this technique the master secret key is negotiated and later used for key derivation. We note this master key as \emph{auth\_key}.

During the DHKE phase, both sides need to generate a random integer \emph{a} and \emph{b}. Worth mentioning, the client generates its \emph{a} in the following way:
\begin{gather*} 
a = r_{client} \oplus r_{server}
\end{gather*}

Where $r_{client}$ is a 2048-bit random integer generated on the client side and $r_{server}$ is 2048-bit random integer generated on the server side. These values are then XORed constructing the client's secret \emph{a} value. This is done to mitigate the client's poor capabilities to generate a cryptographically secure random number. 
%This concerned Android back in August 2013 \cite{telegram-android-securerandom}.
...
\subsubsection{Encryption}\label{telegram-enc}
...
\begin{figure}[htb]
	...
	\caption{MTProto decryption flow \cite{telegram-aarhus}}
