...
Some details may be omitted for simplification. In Point \ref{crypto-initialization-6} the client should check that $p$ and $g$ follow these requirements:\label{crypto-prime-req}
...
The requirements check might be cached and the client is as well allowed to hard-code already checked $p$ and $q$ values into the application.
...
First, let's describe the content of a single message, further referred to as a payload. The payload prior to encryption is depicted on Figure \ref{img:regular-payload} and contains:
...
\begin{itemize}
	\item \texttt{salt} periodically changed number used for replay attacks protection and other
	\item \texttt{session\_id} to identify the user and its device
	\item \texttt{msg\_id} unique ID of the message within a session
	\item \texttt{seq\_no} message sequence counter
	\item \texttt{length} length of the actual message
	\item \texttt{message} text of the actual message
\end{itemize}

\subsection{Encryption}\label{crypto-regular-enc}

The whole encryption process is visualized on Figure \ref{img:regular-enc}.
...
\section{Secret chats}\label{secretchats}

Secret chats are designed to bring an extra level of security compared to regular chats. Secret chat may be initiated between two particular devices only and therefore the messages may be read only on those.

It is crucial to note that all secret chat communication is done using the connection described previously in \ref{crypto-initialization}. All data as described in the following sections are considered as input to the MTProto protocol for regular chats. Hence the data are actually encrypted twice. More on that in TODO

\subsection{Key exchange}\label{crypto-keyexchange}

The exchange performs traditional Diffie-Hellman key exchange (DH). The DH parameters are received from the Telegram server and the client verifies them in the very same manner as described in Section \ref{prime-req}.

Now suppose user $A$ initiates secret chat with $B$:\footnote{Vynechat?}
...
\subsection{Encryption}\label{secret-enc}

TODO

The client sends the message via the \textbf{messages.sendEncrypted} method providing these three parameters:
...
\chapter{new?}\label{code}

The official Telegram application for Android is publicly available on Github \cite{github-telegram} with the first commit dating back to late October 2013.

This thesis concerns the latest Telegram version 3.13.1 as up to date in October 2016, the \texttt{64e8ec3} commit in particular.
...
It should be noted that since this was introduced in the 3.2.2 version (September 2015) where the \cite{telegram-aarhus} analysis is based on 2.7.0 (April 2015) and therefore wasn't concerned with the tgnet library whatsoever.
