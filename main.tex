...
\begin{figure}[htb]
	...
	\caption[WhatsApp's registration process]{WhatsApp's registration process}
...
\begin{figure}[htb]
	...
	\caption[WhatsAppSniffer]{WhatsAppSniffer \cite{whatsapp-sniffer2}}
...
\begin{figure}[htb]
	...
	\caption[Message payload in regular chats]{Payload of a single message to be encrypted in regular chat.}
...
\begin{figure}[htb]
	...
	\caption[IGE block cipher mode]{IGE mode of operation with two Initialization Vectors \cite{telegram-aarhus} used with AES.}
...
\begin{figure}[htb]
	...
	\caption[Encryption key visualization]{Encryption key visualization}
...
\begin{figure}[htb]
	...
	\caption[Message payload in secret chats]{Payload of a single message in secret chat \cite{telegram-aarhus}.}
...
During the analysis we as well scrutinized some parts of the code attempting to discover some potential vulnerabilities to exploit. To provide a little bit of a context let's now briefly describe how Telegram processes any incoming data.

\subsection{Incoming data processing}

First, the \texttt{Connection::onReceivedData()} function is called and the incoming data are deobfuscated in the way described in Section \ref{analysis-obf}.

Next, the \texttt{ConnectionsManager::onConnectionDataReceived()} function is called. If the \texttt{auth\_key\_id} is not set to 0 (which is only the case of the handshake process; see Section \ref{crypto-keyexchange}) the \texttt{Datacenter::decryptServerResponse()} is invoked. This function checks the \texttt{auth\_key\_id} and decrypts the actual payload using the master secret \texttt{auth\_key} and derived \texttt{msg\_key}.

If the decryption succeeds the \texttt{onConnectionDataReceived()} function proceeds and processes the IGE decrypted payload and number of checks are carried out. Finally, if those checks succeed the \texttt{ConnectionsManager::processServerResponse} is called to further process the message.
...
We analyzed how Telegram deals with this issue, see Listing \ref{lst:analysis-replay-check}. Upon the decryption it checks if the message was already processed. The \texttt{ConnectionSession} holds an internal array of already processed messages. If the message is accepted and processed the message ID is added into the array.
...
The behavior we believed might be exploited was the \texttt{addProcessedMessageId} function. The function checks the size of the array and if exceeds 300 it erases first 100 messages as seen in Listing \ref{lst:analysis-replay-add-processed}.


\begin{listing}[htb]
\caption{Telegram source code commit \texttt{abdef}, file Connection.cpp, line 39}
...
If Telegram does not provide any additional checks and actually deletes the first ID the message would be accepted twice. With the attack scenario drafted it is desired to perform the attack.
...
Further, since Telegram uses its own Application layer protocol we were unable to use popular tools such as Fiddler, Burp Suite or OWASP ZAP which are built for HTTP \cite{fiddler-homepage}\cite{burp-homepage}.
...
The scenario was finally tested with Trudy software which can modify any TCP traffic. As its documentation states Trudy is \emph{``a transparent proxy that can modify and drop traffic for arbitrary TCP connections''} \cite{github-trudy}. All traffic is routed through Trudy which then applies so called \emph{modules}. Trudy provides an example of such module and users are encouraged to further implement it. Since Trudy is written in Go all modules are to be written in Go as well. To route the traffic and setup the environment properly a provided virtual machine may be used \cite{github-trudy-vm}.

Upon setting up the MiM as described in \ref{analysis-setup} a virtual machine is run on the computer using Vagrant\footnote{Vagrant is simple wrapper around a virtualization provider for building complete virtual environment.}. The VM installs all required software and Trudy as well. Then it routes all the traffic into and out of Trudy using \texttt{iptables}. Trudy receives all the traffic, modifies it if desired and shifts it back to internet as seen in Figure \ref{img:analysis-trudy-setup}.
...
\begin{wrapfigure}{r}{0.45\textwidth}
	...
	\caption[Trudy's data flow]{Trudy's data flow}
...
First, to test our settings we set up a breakpoint on the very line where Telegram checks if the ID was already processed or not, that is the line 729 and 859 in the \texttt{ConnectionsManager} file. If the packet was actually send again these breakpoints would be fired and the message further rejected by Telegram. This would prove our surmise and later improved to comply fully with the scenario.

Unfortunately we were unable to perform this. We confirmed Telegram successfully  receives the repeated traffic but neither of the desired breakpoints were fired. Further analysis showed Telegram incorrectly deobfuscated the traffic. This is due to the fact that during the 300 messages the \texttt{obf\_enc\_key\_bytes} change periodically. The deobfuscated data are therefore completely different and are rejected on different places as nonsense.

The attack still might be possible if we would save the first message in its deobfuscated form, captured the current obfuscation key and obfuscate the data in that way. We were unable to continue this way unfortunately due to time limitations and scope of this work.

TODO jak to dodelat sem nebo jinam?
...
\subsection{Mirroring attack}

A mirroring attack is similar attack when the attacker captures sent data from the user and directly copies the data to him. If a simple counter is used the client is unaware if the message didn't arrive in the exact same time as the counter in time.
