...
\section{Analysis setup}\label{code-setup}

In this section we briefly describe how we proceeded during the code analysis. First, obviously, we cloned the official Telegram repository. Since we realized a simple code exploration is not satisfactory due to the poor code quality we installed the following software.
...
In addition to that the phone was connected via wireless network to the computer which set up a hotstop exclusively for this use. Wireshark was then used to sniff the data sent from the mobile phone.

% bordel prisel ve verzi 3.7.0. a7513b3ba1923113840e78799c4c4132cc7f3116
...
Lint was run using the default settings and yielded 20 errors and 286 warnings. The full output is attached in appendix. % TODO

Android Studio can as well perform code inspections. It runs lint and adds number of checks such as error handling, security, code style, javadoc, spelling etc. It found over 650 000 issues. The results are as well available in the appendix. % TODO
...
\section{Undocumented obfuscation}\label{code-obf}

One of the main findings is the obfuscation method Telegram uses before sending the data to its severs. This functioning is not documented at all in any way on the Telegram web page or in other sources.

The final data to be sent should follow Figure \ref{img:crypto-regular-enc} as official documentation states. However, before sending the data are encrypted one more time with a random key attached in front of the data. Interestingly using AES Counter mode encryption instead of the IGE endorsed by Telegram. This is done in the \texttt{Connection::sendData} function in \path{TMessagesProj/jni/tgnet/Connection.cpp} file. The detailed description follows.

\subsection{Terminology}

Telegram doesn't practise a great job naming variables. We believe a better terminology is needed, hence we are introducing following titles to add a little more clarity. Since the variables are explained into more details in the following sections, reader may skip this list and use it as a reference later on.
...
\subsection{Temporary encryption key}\label{code-obf-enc-key}

The process starts by generating 64 random bytes \texttt{obf\_enc\_key\_bytes}. The first 8 bytes are unused. The bytes 8 -- 40 are used as an encryption key and bytes 41 -- 56 as an IV. Bytes 57 -- 64 are composed of the last 8 bytes of \texttt{obf\_enc\_key\_bytes} encryption of itself. It is unclear what are those bytes used for.

The array is further appended with the length of the packet, yet again encrypted. The final \texttt{obf\_enc\_key\_bytes} to be sent is visualized in Figure \ref{img:code-obfuscation-bytes-sent}.
...
This array of bytes is sent to the server. The real data to be transmitted as expected from the official documentation are then AES-CTR encrypted using the \texttt{obf\_enc\_key} and \texttt{obf\_enc\_iv} and sent.

\subsection{Temporary decryption key}

Besides the \texttt{obf\_enc\_key\_bytes} setup the very same function deals with setting up the \texttt{obf\_dec\_key\_bytes}, noted in Telegram as \emph{temp}. This temporary key is used for decrypting incoming traffic.
...
\begin{lstlisting}[language=C,label=lst:code-obf-dec,caption=Temporary decryption key deduction.]
   for (int a = 0; a < 48; a++) {
      temp[a] = bytes[55 - a];
   }     
\end{lstlisting}
