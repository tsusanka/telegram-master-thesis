...
\abstractEN{This thesis is devoted to an analysis of the Telegram Messenger and the related MTProto protocol. It studies the cryptographic background of MTProto, the Android client source code and the produced traffic. It compares the application with its official documentation. Finally it discusses potential vulnerabilities, their discoveries and various attempts to exploit them.}
...
In this section we briefly look on how Telegram stores its data. All data are stored in the phone's \path{org.telegram.messenger/files} folder. Two types of files are present -- SQLite database file and \path{.dat} files which are in a Telegram's own configuration format:
